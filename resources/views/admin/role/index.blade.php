...
@section('content')
    <div class="pt-3">
        <div class="col-sm-12 ml-1 text-bold mb-1" style="font-size: 20px;">
            <i class="far fa-user-shield text-muted mr-2"></i> {{ $pagename }}
        </div>
        ...
        <div class="card {{ env('CARD_THEME') }} shadow-custom">
            ...
            <div class="card-body">
                ...
                @if (Auth::user()->hasAnyPermission(['*', 'all role', 'create role']))
    ...
    @push('js')
        <script>
            ...
            function requestData(response) {
                $('#eid').val(response.role.id);
                $('#ename').val(response.role.name);
                $('#edescription').val(response.role.description);

                p_ids = [];
                $.each(response.permissions, function(pindex, pvalue) {
                    p_ids.push(pvalue.id);
                });

                $('#epermission').val(p_ids);
                $('#epermission').select2();

                roles_name = ['developer', 'superadmin', 'admin', 'employee'];

                if (roles_name.includes(response.role.name)) {
            ...
            function updateData() {
                ...
                eformdata.append('permission', $('#epermission').val());
    ...
    @include('admin.role.partials.create-modal')
    @include('admin.role.partials.edit-modal')
@endsection
