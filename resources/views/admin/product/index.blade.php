...
@php $pagename = 'จัดการอุปกรณ์'; @endphp
...
@section('content')
    <div class="pt-3">
        ...
        <div class="card {{ env('CARD_THEME') }} shadow-custom">
            ...
            <div class="card-body">
                ...
                @if (Auth::user()->hasAnyPermission(['*', 'all product', 'create product']))
        ...
        <script>
            ...
            function storeData(url) {
                if ($('#name').val() == null || $('#name').val() == "") {
                    toastr.error('กรุณาเพิ่มชื่ออุปกรณ์');
                    return false;
                }

                if ($('#price').val() == null || $('#price').val() == "") {
                    toastr.error('กรุณาเพิ่มราคา');
                    return false;
                }

                if ($('#unit').val() == null || $('#unit').val() == "") {
                    toastr.error('กรุณาเพิ่มหน่วยนับ');
                    return false;
                }

                if ($('#limit').val() == null || $('#limit').val() == "") {
                    toastr.error('กรุณาเพิ่มจำนวนการเบิกสูงสุด');
                    return false;
                }
            ...
            function requestData(response) {
                $('#eid').val(response.product.id);
                $('#ename').val(response.product.name);
                $('#edescription').val(response.product.description);
                $('#eproduct_type_id').val(response.product.product_type_id);
                $('#eprice').val(response.product.price);
                $('#eunit').val(response.product.unit);
                $('#elimit').val(response.product.limit);
            ...
            function updateData() {
                ...
                if ($('#eunit').val() == null || $('#eunit').val() == "") {
                    toastr.error('กรุณาเพิ่มหน่วยนับ');
                    return false;
                }

                if ($('#elimit').val() == null || $('#elimit').val() == "") {
                    toastr.error('กรุณาเพิ่มจำนวนการเบิกสูงสุด');
                    return false;
                }
                ...
                eformdata.append('unit', $('#eunit').val());
                eformdata.append('limit', $('#elimit').val());
                eformdata.append('product_type_id', $('#eproduct_type_id').val());
    ...
    @include('admin.product.partials.create-modal')
    @include('admin.product.partials.edit-modal')
@endsection
